...
export interface ModalProps {
    ...
    /** วิ่งเส้นแบรนด์บนขอบบน (ผลเชิงภาพล้วน ๆ) ไม่แตะว่ากล่องปิดได้หรือไม่ */
    busy?: boolean;
    /** มีคำขอค้างอยู่ (เขียน ไม่ใช่แค่โหลด): กันกล่องปิดไม่ว่าทางไหน — Escape,
        ปุ่ม ✕ (disabled), คลิกพื้นหลัง แยกจาก `busy` โดยตั้งใจ: `busy` ของ
        PalletDetailModal คือ "กำลังโหลดประวัติ" ซึ่งเป็นแค่ read ไม่มีอะไรต้อง
        ป้องกัน ถ้ายังรวมเป็น prop เดียวกัน กล่องนั้นจะปิดไม่ได้เลยระหว่างโหลด
        ทั้งที่ไม่มีคำขอค้างอยู่จริง */
