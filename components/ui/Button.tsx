...
export const BUTTON_VARIANT: Record<ButtonVariant, string> = {
    ...
    // ===== ตระกูล "ปุ่มพื้นขาว" -- secondary / danger / accentSoft ==============
    //
    // ทั้งสามใบใช้พื้นผิวเดียวกันเป๊ะ: พื้นขาว + `shadow-raised` ไม่มีเส้นขอบ
    // ต่างกันแค่ hue ของตัวหนังสือและสีพื้นตอน hover ซึ่งเป็นสิ่งเดียวที่ควรต่าง
    // -- ทั้งสามทำงานเดียวกัน คือ "ปุ่มที่ไม่ใช่ปุ่มหลักของหน้า"
    //
    // ประวัติของสามบรรทัดนี้ยาวกว่าที่ควร และเขียนไว้เพื่อไม่ให้วนกลับไปที่เดิม:
    // เดิมทั้งสามเป็น "พื้นขาว + เส้นขอบ 1px" ซึ่งเส้นวัดได้ 1.23:1 บนการ์ดขาว
    // (แดงได้ 1.7:1) -- ต่ำกว่าเกณฑ์ 3:1 ของ WCAG 1.4.11 มาก และในทางปฏิบัติคือ
    // ปุ่มละลายไปกับพื้น รอบถัดมาลองขอบเข้ม `#7d8ba1` (3.45:1 ผ่านเกณฑ์) แต่หน้าจอ
    // เต็มไปด้วยเส้น รอบถัดมาลองพื้นเทาอ่อน ซึ่งแย่กว่าเดิม: เทาอ่อนบนพื้นหน้าจอ
    // `--color-canvas` วัดได้ 1.03:1 -- จางกว่าตอนเป็นขอบอีก
    //
    // สิ่งที่ทำให้ปุ่มพวกนี้ "เห็นเป็นปุ่ม" ตอนนี้คือเงา ไม่ใช่พื้นและไม่ใช่ขอบ
    // `shadow-raised` เป็นชั้นความสูงชุดเดียวกับการ์ด (index.css) -- ปุ่มจึงเป็น
    // "แผ่นเล็กที่ลอยอยู่เหนือพื้น" วัสดุเดียวกับการ์ดที่มันวางอยู่บน อย่าถอดเงาออก
    // เพราะเห็นว่า "ปุ่มไม่น่าต้องมีเงา": ถ้าถอด ปุ่มขาวบนการ์ดขาวจะเหลือ 1:1 พอดี
    // คือไม่มีอะไรเหลือให้เห็นเลย
    //
    // ข้อจำกัดที่ยังอยู่ ต้องรู้ไว้: เงาไม่ใช่ "สี" ที่ WCAG 1.4.11 วัดได้ ตระกูลนี้
    // จึงยังไม่ผ่านเกณฑ์ 3:1 อย่างเป็นทางการ มันเป็นการแลกที่เจ้าของงานเลือกเอง
    // ระหว่างหน้าจอที่สะอาดกับตัวเลขที่ผ่าน -- ทางที่ผ่านคือใส่เส้นขอบที่เข้มพอ
    // (`#7d8ba1` ขึ้นไป) กลับเข้ามา ไม่ใช่โทเคน `line-control` ที่ใช้อยู่ตอนนี้:
    // ค่านั้นก็ถูกลดลงมาให้กลมกลืนแล้วเหมือนกัน -- ดูคอมเมนต์ของมันใน index.css
    secondary:
        'bg-white text-slate-700 shadow-raised hover:bg-slate-50 ' +
    ...
    //
    // สัญญาณ "อันตราย" อยู่ที่ตัวหนังสือแดงกับพื้นแดงจางตอน hover ไม่ใช่ที่เส้นขอบ
    // อีกแล้ว -- เส้น `border-red-200` เดิมวัดได้ 1.7:1 บนขาว ซึ่งเป็นสัญญาณที่
    // แทบไม่มีใครเห็นอยู่แล้ว ส่วนตัวหนังสือ `red-600` ได้ 4.9:1
    danger:
        'bg-white text-red-600 shadow-raised hover:bg-red-50 ' +
        'active:scale-[0.99] disabled:hover:bg-white',
    ...
    // ปุ่ม `primary` ของหน้า แล้วยอมถอยให้ปุ่มหลักเด่นกว่า
    //
    // ตอนนี้ไม่มีใครใช้ variant นี้แล้ว -- ผู้ใช้ทั้งสองรายเคยมีจริงและถูกย้ายไป `accent`
    // ทึบตามที่เจ้าของงานสั่ง ด้วยเหตุผลเดียวกันทั้งคู่: ปุ่มพื้นขาวใบเล็กบนการ์ดขาวหรือ
    // บนหัวเพจอ่านไม่ออกว่าเป็นปุ่ม
    //   - "พิมพ์ QR" บนหัวหน้าคลัง -> ย้ายเพื่อให้สีตรงกับปุ่ม "พิมพ์ / PDF" ในโมดัลที่มัน
    //     เปิดขึ้นมา ผลคือหัวหน้าคลังมีปุ่มทึบสองสี (accent + primary) นั่งข้างกัน ตั้งใจ
    //   - "ทดสอบ" ในหน้าตั้งค่าแจ้งเตือน -> ย้ายเพื่อให้เป็นสีเดียวกับ "พิมพ์ QR"
    //
    // ยังไม่ลบทิ้ง เพราะตระกูลปุ่มพื้นขาวเหนือขึ้นไปมีสามใบที่ต้องเป็นชุดเดียวกัน
    // (secondary / danger / accentSoft) การเอาออกใบเดียวคือเปิดช่องให้คนถัดไปที่ต้องการ
    // ปุ่มขาวโทน accent ไปประกอบสีขึ้นใหม่เอง ซึ่งเป็นปัญหาที่ตารางนี้มีอยู่เพื่อแก้
    accentSoft:
        'bg-white text-accent-700 shadow-raised hover:bg-accent-50 ' +
        'active:scale-[0.99] disabled:hover:bg-white',
