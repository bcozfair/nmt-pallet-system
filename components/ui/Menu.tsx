...
// `w-max` ไม่ใช่ `w-64`: พาเนลกว้างเท่าที่รายการยาวที่สุดต้องการ ไม่ใช่ 256px ตายตัว
//
// ค่าเดิมเป็นค่าที่เผื่อไว้สำหรับเมนูที่ยาวที่สุดในแอป แล้วบังคับใช้กับทุกเมนู เมนู
// "ล้างข้อมูล" ที่มีรายการเดียวจึงกว้าง 256px เท่ากับเมนูส่งออกสามรายการ -- กล่องว่าง
// ครึ่งใบที่ไม่มีอะไรอยู่ข้างใน
//
// สองค่าที่เหลือเป็นขอบเขตของ `w-max` ไม่ใช่การตกแต่ง:
//   min-w-36 (144px) กันไม่ให้เมนูที่ป้ายสั้นมากหดจนกลายเป็นแถบผอม ๆ ที่กดยาก
//   max-w กันไม่ให้ล้นจอ พาเนลเกาะกับปุ่มที่อาจอยู่ชิดขอบจอ และ `w-max` ไม่มีเพดาน
//         ของตัวเอง ป้ายภาษาไทยที่ยาวกว่าอังกฤษจะดันจนห้อยพ้นจอ พอ max-w เข้ามา
//         ตัด ข้อความจะกลับไปตัดบรรทัดตามปกติ ไม่ได้ถูกซ่อน
const PANEL_WIDTH_DEFAULT = 'w-max min-w-36 max-w-[calc(100vw-2rem)]';
