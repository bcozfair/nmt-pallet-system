    ...
    return createPortal(
        ...
        >
            ...
            >
                ...
                {footer && (
                    // พื้นและเส้นชุด `surface-band` ชุดเดียวกับท้ายตาราง (Pagination.tsx)
                    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                    // ค่าเดิม `bg-slate-50/70` ทับพื้นขาวของโมดัลออกมาเป็น `#fafcfd`
                    // ซึ่งแทบเป็นขาว ปุ่มพื้นขาว (`secondary` / `danger`) ที่วางอยู่บนนั้น
                    // จึงได้คอนทราสต์ 1.03:1 -- เหลือแค่เงาบาง ๆ เป็นสิ่งเดียวที่บอกว่า
                    // มีปุ่มอยู่ ซึ่งเป็นอาการเดียวกับที่ท้ายตารางเคยเป็นเป๊ะ ๆ
                    //
                    // ทางแก้ไม่ใช่การเพิ่มเงาให้ปุ่ม: เงาที่หนักขึ้นบนพื้นเกือบขาวก็ยัง
                    // อ่านยากอยู่ดี และจะทำให้ปุ่มในโมดัลเป็นวัตถุคนละชนิดกับปุ่มใบเดียวกัน
                    // ที่อยู่นอกโมดัล ปัญหาอยู่ที่พื้นที่มันวางอยู่ ไม่ใช่ที่ตัวปุ่ม
                    //
                    // บนแถบใหม่: ปุ่มพื้นขาวได้ 1.29:1 และปุ่มแดงทึบ (`dangerSolid`) ได้
                    // 3.73:1 ซึ่งทั้งคู่ไม่ต้องแก้อะไรเลย
                    <div className="flex shrink-0 flex-wrap items-center justify-end gap-3 border-t border-line-band bg-surface-band px-5 py-4">
                        {footer}
                    </div>
                )}
            </div>
